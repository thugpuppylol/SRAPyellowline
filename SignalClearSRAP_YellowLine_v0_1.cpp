...
static float FindMostRecentNonZero(
    ...
    int           LookbackMax)
{
    int Limit = CurrentIndex - LookbackMax;
    if (Limit < 0) Limit = 0;
...
SCSFExport scsf_SignalClearSRAP_YellowLine_v0_1(SCStudyInterfaceRef sc)
{
    ...
    //
    // Write all values to subgraph arrays every bar.
    // SG0-SG3: always written (spreadsheet-ready).
    // SG4-SG7: debug — written when ShowDebug is enabled.
    //
    // Validation checklist:
    //   SG4 (Anchor): should match ZZ reversal prices visually on chart
    //   SG5 (ATR):    should be ~20-25 pts on NQ 20R (raw), not scaled
    //   SG6 (Changed):spikes of 1.0 should align with visible line moves
    ...
    // Debug subgraphs — written regardless of ShowDebug; user controls visibility
    // via subgraph draw style settings. Written in all modes for replay validation.
