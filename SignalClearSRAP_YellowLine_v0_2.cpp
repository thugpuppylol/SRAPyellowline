// =============================================================================
// SignalClearSRAP_YellowLine_v0_1.cpp
...
SCDLLName("SignalClear SRAP Yellow Line v0.1")
...
// =============================================================================
SCSFExport scsf_SignalClearSRAP_YellowLine_v0_1(SCStudyInterfaceRef sc)
{
    // ── Subgraph references ───────────────────────────────────────────────────
    ...
    SCSubgraphRef SG_DBG_Source  = sc.Subgraph[7]; // Debug: 0=ZZ 1=POC 2=HybZZ 3=HybPOC
    ...
    if (sc.SetDefaults)
    {
        sc.GraphName        = "SignalClear SRAP Yellow Line v0.1";
        sc.StudyDescription =
        ...
        SG_DBG_Source.Name         = "DBG: Anchor Source (0=ZZ 1=POC 2=HybZZ 3=HybPOC)";
        ...
        In_AnchorMode.Name         = "Anchor Mode: 0=ZZ  1=MicroPOC  2=Hybrid";
        In_AnchorMode.SetInt(0);
        In_AnchorMode.SetIntLimits(0, 2);
        ...
        // ATR period: SMA of True Range over this many bars
        // Matches the real study's 5-min ATR(24, SMA) conceptually
        In_ATRLength.Name          = "ATR Period (bars, SMA of True Range)";
        In_ATRLength.SetInt(24);
        In_ATRLength.SetIntLimits(2, 500);

        // ATR multiplier k: half-band = k × ATR
        // On NQ 20R: TR≈20 pts, SMA(TR,24)≈22 pts
        // k=4.5 → half-band ≈ 99 pts (within observed 65-151 pt range)
        // Tune upward if band appears too narrow; downward if too wide
        In_ATRMultiplier.Name      = "ATR Band Multiplier k (half-band = k × ATR)";
        In_ATRMultiplier.SetFloat(4.5f);
    ...
    // Initialize line numbers on full recalculation (chart open, input change)
    // Use StudyGraphInstanceID to generate unique line numbers per study instance
    // preventing collisions if this study is added to multiple charts
    if (sc.IsFullRecalculation && sc.Index == 0)
    {
        LastDrawnAnchor = 0.0f;
        const int Base  = 4000 + (sc.StudyGraphInstanceID % 200) * 10;
        SRAPLineNum     = Base + 1;
        LabelLineNum    = Base + 2;

        // Delete any existing drawings from prior sessions/runs
        sc.DeleteACSILDrawingTool(SRAPLineNum);
        sc.DeleteACSILDrawingTool(LabelLineNum);
    }

    // Guard: if line numbers are still 0 (e.g. after DLL reload without full recalc)
    if (SRAPLineNum == 0)
    {
        const int Base  = 4000 + (sc.StudyGraphInstanceID % 200) * 10;
        SRAPLineNum     = Base + 1;
    ...
    float AnchorSource      = -1.0f; // -1=none, 0=ZZ, 1=POC, 2=HybridZZ, 3=HybridPOC

    if (AnchorMode == 0)
    {
        // Pure ZZ: use last ZZ reversal price
    ...
    else // AnchorMode == 2: Hybrid
    {
        // Base: ZZ reversal price
        // Snap: if POC is valid AND within ProxPoints of ZZ anchor, use POC
        if (ZZAnchor != 0.0f && POCAnchor != 0.0f &&
            std::fabs(POCAnchor - ZZAnchor) <= ProxPoints)
        {
            // POC is close to ZZ anchor — snap to POC for precision
            FinalAnchor  = POCAnchor;
            AnchorSource = 3.0f; // Hybrid locked on POC
        }
        else if (ZZAnchor != 0.0f)
        {
            // No POC in proximity — stay on ZZ anchor
            FinalAnchor  = ZZAnchor;
            AnchorSource = 2.0f; // Hybrid staying on ZZ
        }
        else if (POCAnchor != 0.0f)
        {
            // ZZ not available — fall back to POC alone
            FinalAnchor  = POCAnchor;
            AnchorSource = 1.0f;
        }
        // else: no anchor available, FinalAnchor stays 0
    }
    ...
    //
    // Sum (AskVolume - BidVolume) over the last DeltaWindow bars.
    // Positive = buyers pressing harder. Negative = sellers pressing harder.
    // This is the "Δ" value displayed beside the yellow line.
    // =========================================================================
    ...
    //
    // Validation target: with k=4.5 on NQ 20R, band half-width should be
    // roughly 65-151 pts, matching observed real data.
    // Use DBG subgraph SG5 to verify ATR value is in expected range.
    // =========================================================================
    const int   ATRLength = In_ATRLength.GetInt();
    const float ATRVal    = ComputeApproxATR(sc, Idx, ATRLength);
    ...
    //   SG5 (ATR):     should be ~20-25 pts on NQ 20R; verify half-band
    //                  = ATRMultiplier × SG5 matches (SG2 - SG3) / 2
    ...
    //
    // Decide whether to redraw the line and label.
    //
    // Redraw condition: anchor has shifted by >= UpdateThresholdTicks since
    // last draw. This filters out float-precision noise while still tracking
    // the rolling anchor as the market shifts structure.
    //
    // Always redraw on the last (live) bar to keep the line current.
    //
    // FIX (audit issue #2 continued):
    //   SG6 (AnchorChanged) is now written here, after ShouldDraw is known,
    //   and only when ShowDebug is enabled.
    //
    // Important: UTAM_ADD_OR_ADJUST means Sierra Chart will move/update the
    // existing tool rather than creating a duplicate. The single yellow line
    // (SRAPLineNum) is updated in place every time the anchor changes.
    // This means only ONE yellow line is visible at a time — the current one.
    // Historical anchor levels are available via the SG_SRAPAnchor array.
    // =========================================================================

    // ── Fail-fast config sanity checks (logged once per bar) ───────────────
    // Catches common setup mistakes before the user wonders why nothing draws.
    if (AnchorMode == 0 && ZZStudyID == 0)
    {
        sc.AddMessageToLog(
            "SignalClear SRAP: AnchorMode=ZZ but ZigZag Study ID is 0. "
            "Set ZigZag Study ID to the ID of your built-in ZZ study.", 0);
        return;
    }
    if (AnchorMode == 1 && POCStudyID == 0)
    {
        sc.AddMessageToLog(
            ...
            "Set MicroPOC Study ID to your SignalClear MicroVPOC study ID.", 0);
        return;
    }
    if (AnchorMode == 2 && ZZStudyID == 0 && POCStudyID == 0)
    {
        sc.AddMessageToLog(
            ...
            "Set at least ZigZag Study ID.", 0);
        return;
    }
    ...
    const bool ShouldDraw    = AnchorChanged || IsLastBar;

    // Write SG6 now that ShouldDraw is determined (gated behind ShowDebug)
    if (In_ShowDebug.GetYesNo())
        SG_DBG_Changed[Idx] = ShouldDraw ? 1.0f : 0.0f;

    if (!ShouldDraw)
        return; // Nothing to redraw; subgraph values already written above

    // Update the recorded anchor
    LastDrawnAnchor = FinalAnchor;


    // ── Draw yellow dashed SRAP horizontal line ────────────────────────────
    {
        s_UseTool LineTool;
        LineTool.Clear();
        ...
        LineTool.Color             = RGB(255, 220, 0); // Yellow
    ...
    // ── Draw delta label text beside the line ─────────────────────────────
    if (In_ShowDeltaLabel.GetYesNo())
    {
        const float MinDelta = In_MinDeltaAbs.GetFloat();

        // Format delta string: "+123" for positive, "-123" for negative
        // Note: DeltaSum is truncated to int (not rounded) — consistent with
        // observed real-study label behavior (whole numbers, no decimals)
        char DeltaBuf[32];
        if (DeltaSum >= 0.0f)
            sprintf(DeltaBuf, "+%d", static_cast<int>(DeltaSum));
        else
            sprintf(DeltaBuf, "%d",  static_cast<int>(DeltaSum));

        // Position: slightly above the SRAP line (3 ticks up)
        const float LabelPrice = FinalAnchor + 3.0f * sc.TickSize;

        // Color: full yellow if delta is meaningful; dimmed if below threshold
        // This lets the line always show but signals confidence in the label
        const COLORREF LabelColor = (std::fabs(DeltaSum) >= MinDelta)
            ? RGB(255, 220, 0)   // Bright yellow — significant delta
            : RGB(140, 120, 30); // Dim gold — weak/noisy delta

        s_UseTool LabelTool;
        LabelTool.Clear();
        LabelTool.ChartNumber   = sc.ChartNumber;
        LabelTool.LineNumber    = LabelLineNum;
        LabelTool.DrawingType   = DRAWING_TEXT;
        LabelTool.AddMethod     = UTAM_ADD_OR_ADJUST;
        LabelTool.BeginDateTime = sc.BaseDateTimeIn[Idx];
        LabelTool.Price1        = LabelPrice;
        LabelTool.Text          = DeltaBuf;
        LabelTool.Color         = LabelColor;
        LabelTool.FontSize      = 10;
        LabelTool.FontBold      = true;

        sc.UseTool(LabelTool);
    }
    else
    {
        // FIX (audit issue #3 — MEDIUM):
        // ShowDeltaLabel was toggled off. If a label was previously drawn it
        // will linger as a stale text tool on the chart. Delete it explicitly.
        // sc.DeleteACSILDrawingTool is a no-op if the tool doesn't exist,
        // so this is safe to call on every bar when the toggle is off.
        sc.DeleteACSILDrawingTool(LabelLineNum);
    }

    // End of scsf_SignalClearSRAP_YellowLine_v0_1
}
